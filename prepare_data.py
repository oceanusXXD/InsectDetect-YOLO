...
def prepare_classification_dataset(source_dir, output_dir):
    ...
    print(f"[INFO] Classification dataset ready: {output_path} | Classes={len(classes)}")


def _process_class_folders(source_path: Path, output_path: Path, classes, image_label_pairs):
    print("[INFO] Detected 'class-folder structure', processing...")
    ...
    train, val = train_test_split(train_val, test_size=0.176, random_state=42)  # 0.176*0.85 approx 0.15
...
def _process_split_dataset(source_path: Path, output_path: Path, classes):
    print("[INFO] Detected 'train/val/test split structure', processing (full-image classification)...")
    for split in ['train', 'val', 'test']:
...
def _copy_and_label_images(pairs, dest_path: Path, split_name: str):
    print(f"[INFO] Processing {split_name}: {len(pairs)} images")
...
def _save_cls_yaml(output_path: Path, classes):
    ...
    print(f"[INFO] data.yaml saved: {output_path / 'data.yaml'}")


if __name__ == '__main__':
    import argparse
    ap = argparse.ArgumentParser(description="Class Folders -> YOLO-CLS Directory Structure")
